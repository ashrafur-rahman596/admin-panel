...
s_a[38]="Alberta|British Columbia|Calgary|Manitoba|Markham|New Brunswick|Newfoundland|Northwest Territories|Nova Scotia|Nunavut|Ontario|Prince Edward Island|Pickering|Quebec|Saskatchewan|Toronto|Yukon Territory|Vancouver";
...
function print_country(country_id, selected_value){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var option_str = document.getElementById(country_id);
	option_str.length=0;
	option_str.options[0] = new Option('Select Country','');
	option_str.selectedIndex = 5;
	for (var i=0; i<country_arr.length; i++) {
		option_str.options[option_str.length] = new Option(country_arr[i],country_arr[i]);
        if(selected_value==country_arr[i])
        {
...
//        if(selected_value==state_arr[i])
//        {
//            option_str.selectedIndex = option_str.length;
//        }
	}
}

function print_city(state_id, country_value, city_value){
    var option_str = document.getElementById(state_id);
    option_str.length=0;	// Fixed by Julian Woods
    option_str.options[0] = new Option('Select State','');
    option_str.selectedIndex = 0;
    var state_index;
    for (var arr_count=0; arr_count<country_arr.length; arr_count++)
    {
        if(country_value==country_arr[arr_count])
        {
            state_index = (arr_count+1);
            //break;
        }
    }
